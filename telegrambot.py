...
def start(message):
    ...
    dataBase_manager.save_city(user.id, ' ')
...
def response(message):
    ...
    if message.text == 'Выбрать действие':
        ...
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
        city = dataBase_manager.get_city(user.id)[0][0]
        print(city)
        if len(city) < 2:
            bot.reply_to(message, 'У вас не задан город!', reply_markup=markup)
        else:
            resp = get_weather(city)
            print(resp)
            strrr = format_weather(resp)
            bot.reply_to(message, strrr, reply_markup=markup)
